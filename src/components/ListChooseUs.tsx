...
import { chooseUpData } from "@/data/choosUsData";
  ...
  export function ListChooseUs(   ) {
    return (
      ...
      <MediaQuery maxWidth={1279.9}>
          ...
          {chooseUpData.map(({ title, text }, ind) => (
            <li key={ind}  
             >
           
              <h4  className={`text-sm font-normal mb-2 tab:text-base    } `} > {title}</h4>
